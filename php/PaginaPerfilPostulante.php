    ...
    if ($_FILES["image"]["error"] == 4) {
        ...
        if (!in_array($imageExtension, $validImageExtension)) {
            ...
            if (move_uploaded_file($tmpName, '../img/' . $newImageName)) {
                ...
                $query = "UPDATE postulante SET Foto = '../img/$newImageName' WHERE IdUsuario = $IdUsuario";
                if (mysqli_query($conexion, $query)) {
                    echo "<script>alert('Imagen actualizada con éxito'); document.location.href = 'PaginaPerfilPostulante.php';</script>";
                } else {
...
if (isset($_POST["submit_cv"])) {
    if ($_FILES["cv"]["error"] == 4) {
        ...
        $validFileExtension = ['pdf', 'jpg', 'docx','png'];
        $fileExtension = explode('.', $fileName);
        $fileExtension = strtolower(end($fileExtension));
        if (!in_array($fileExtension, $validFileExtension)) {
            echo "<script>alert('Extensión de archivo inválida');</script>";
        } else if($fileSize > 10000000) {  // 10 MB
            echo "<script>alert('El tamaño del archivo es muy grande');</script>";
        } else {
            $newFileName = uniqid();
            $newFileName .= '.' . $fileExtension;

            if (move_uploaded_file($tmpName, '../img/CV/' . $newFileName)) {
                // Eliminar el archivo CV anterior si existe
                if (!empty($info_adicional['CV']) && file_exists($info_adicional['CV'])) {
                    unlink($info_adicional['CV']);
                }

                // Actualizar la base de datos con la nueva URL del archivo CV
                $query = "UPDATE postulante SET CV = '../img/CV/$newFileName' WHERE IdUsuario = $IdUsuario";
                if (mysqli_query($conexion, $query)) {
                    echo "<script>alert('CV actualizado con éxito'); document.location.href = 'PaginaPerfilPostulante.php';</script>";
                } else {
                    echo "<script>alert('Error al actualizar la base de datos');</script>";
                }
            } else {
...
if (isset($_POST["submit_info"])) {
    ...
    $query = "UPDATE postulante SET Nombre='$nombre', ApellidoP='$apellidoP', ApellidoM='$apellidoM', Celular='$celular', Direccion='$direccion', Descripcion='$descripcion' WHERE IdUsuario = $IdUsuario";
    ...
    if (mysqli_query($conexion, $query) && mysqli_query($conexion, $query2)) {
        echo "<script>alert('Información actualizada con éxito'); document.location.href = 'PaginaPerfilPostulante.php';</script>";
    } else {
...
if (isset($_POST["submit_servicio"])) {
    ...
    // Configurar la zona horaria a la hora local de Bolivia (Santa Cruz)
    ...
    if (mysqli_query($conexion, $query_servicio)) {
        ...
        $query_publicacion = "INSERT INTO publicaciones (IdServicios, IdUsuario, Fecha, DescripcionPublicacion, TipoPublicacion) VALUES ('$id_servicio', '$IdUsuario', '$fecha', '$descripcion', 'Servicio')";
        if (mysqli_query($conexion, $query_publicacion)) {
            echo "<script>alert('Servicio publicado con éxito'); document.location.href = 'PaginaPerfilPostulante.php';</script>";
        } else {
            echo "<script>alert('Error al publicar el servicio');</script>";
        }
    } else {
        echo "<script>alert('Error al registrar el servicio');</script>";
    }
}
...
if (isset($_POST["delete_publicacion"])) {
    ...
    if (mysqli_query($conexion, $query_delete)) {
        echo "<script>alert('Publicación eliminada con éxito'); document.location.href = 'PaginaPerfilPostulante.php';</script>";
    } else {
...
if (isset($_POST["edit_publicacion"])) {
    ...
    if (mysqli_query($conexion, $query_edit)) {
        echo "<script>alert('Publicación editada con éxito'); document.location.href = 'PaginaPerfilPostulante.php';</script>";
    } else {
...
<nav class="navbar navbar-expand-lg navbar-light bg-light">
    <div class="container">
        <a class="navbar-brand" href="PaginaPerfilPostulante.php">MENU</a>
        <button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#navbarNav" aria-controls="navbarNav" aria-expanded="false" aria-label="Toggle navigation">
            <span class="navbar-toggler-icon"></span>
        </button>
        <div class="collapse navbar-collapse" id="navbarNav">
            <ul class="navbar-nav ml-auto">
                <li class="nav-item"><a class="nav-link" href="../php/PaginaPublicacionesOfertas.php">Publicaciones</a></li>
                    ...
                    <div class="cv-content text-center">
                        <?php if (!empty($info_adicional['CV'])): ?>
                            <a href="#" data-toggle="modal" data-target="#uploadCvModal">
                                <img src="../img/miniatura-cv.png" alt="Miniatura del CV" class="img-thumbnail" width="200px" height="auto">
                            </a>
                        <?php else: ?>
                            <a href="#" data-toggle="modal" data-target="#uploadCvModal">
                                <p>Click aquí para subir un CV</p>
                            </a>
                        <?php endif; ?>
                    </div>
                    <?php if (!empty($info_adicional['CV'])): ?>
    ...
    <div class="modal fade" id="uploadCvModal" tabindex="-1" role="dialog" aria-labelledby="uploadCvModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="uploadCvModalLabel">Subir CV</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <form action="" method="post" enctype="multipart/form-data">
                        <div class="form-group">
                            <label for="cv"style="color: black;">Seleccionar archivo PDF:</label>
                            <input type="file" name="cv" id="cv" class="form-control" accept=".pdf, .doc, .docx" required>
                        </div>
                        <button type="submit" name="submit_cv" class="btn btn-primary">Subir</button>
                    </form>
                </div>
            </div>
        </div>
                ...
                <div class="modal-body">
                    <form action="" method="post">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="apellidoM" name="apellidoM" value="<?php echo $info_adicional['ApellidoM']; ?>" required>
                        </div>
                        <div class="form-group">
    ...
    <div class="modal fade" id="publicarModal" tabindex="-1" role="dialog" aria-labelledby="publicarModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="publicarModalLabel">Publicar Servicio</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
